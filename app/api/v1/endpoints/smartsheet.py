...
@router.get(
    "/health",
    summary="Smartsheet service health check",
    description="Checks if the Smartsheet service is healthy and can connect to the API"
)
async def health_check(
    ...
    """
    Endpoint de health check para el servicio de Smartsheet
    ...
    """
    try:
        logger.info("GET /smartsheet/health")

        health_status = await smartsheet_service.health_check()

        if health_status.get("status") == "healthy":
            logger.info("Smartsheet health check passed")
            return health_status
        else:
            logger.warning(f"Smartsheet health check failed: {health_status}")
            raise HTTPException(
