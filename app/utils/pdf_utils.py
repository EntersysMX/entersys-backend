...
def generate_certificate_pdf(
    ...
    # === RESULTADOS POR SECCION (si hay datos) ===
    if section_results:
        elements.append(Paragraph("Resultados por Secci\u00f3n", heading_style))

        yellow_line2 = Table([['', '']], colWidths=[width])
        yellow_line2.setStyle(TableStyle([
            ('LINEBELOW', (0, 0), (-1, 0), 2, COLOR_YELLOW),
        ]))
        elements.append(yellow_line2)
        elements.append(Spacer(1, 8))
        ...
        for key, name in section_names.items():
            score_val = section_results.get(key)
            if score_val is not None:
                try:
                    score_num = float(str(score_val).replace('%', '').strip())
                except (ValueError, TypeError):
                    score_num = 0

                approved_section = score_num >= 80
                estado_text = "Aprobada" if approved_section else "Reprobada"
                estado_color = COLOR_GREEN if approved_section else COLOR_RED
                ...
                score_data.append([
                    Paragraph(name, ParagraphStyle('SectionName', parent=styles['Normal'], fontSize=10, alignment=TA_CENTER)),
                    Paragraph(f"{score_num:.0f}%", ParagraphStyle('ScoreVal', parent=styles['Normal'], fontSize=11, fontName='Helvetica-Bold', alignment=TA_CENTER)),
                    Paragraph(estado_text, estado_style),
                ])

        score_table = Table(score_data, colWidths=[width / 3] * 3)
